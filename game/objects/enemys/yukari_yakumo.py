    ...
    def draw(self,screen):
        ...
        if self.current_action == "ATTACK_4":
            if self.pattern_data:
                sprite = self.game.sprites["rift_1"]
                for x,y in self.pattern_data:
                    screen.blit(sprite["image"], (x - self.game.camera.x - sprite["center"][0], y - self.game.camera.y - sprite["center"][1]))
    def death(self):
        super().death();
        ...
        if self._tick % 4 == 0:
            ...
            if self.current_action == "ATTACK_1":
                ...
                for i in range(num_bullets):
                    ...
                    b.set_homing(self.game.player, 100)
                    b.set_f(1.1)
                ...
                is_shoot = True
                self.x = self.game.player.x + random.randint(-750, 750)
                self.y = self.game.player.y + random.randint(-550, 550)
                b = self.new_bullet()
                b.set_position(self.x, self.y)
                dx, dy = get_direction_to([b.x, b.y], [self.game.player.x, self.game.player.y])
                ...
                if self.pattern_data == None:
                    self.pattern_data = [(self.game.player.x + random.randint(-750, 750), self.game.player.y + random.randint(-550, 550)) for _ in range(20)]
                ...
                for _ in range(2):
                        chain_x = self.game.camera.x - 100
                        chain_y = self.game.camera.world_pos["y"] + random.randint(-750, 750)
                        for i in range(5):
                            b = self.new_bullet()
